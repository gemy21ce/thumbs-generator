...
function thumb ($src_file, $width = false, $height = false, $crop = false, $format = false)
{
	...
	$thumb_file = THUMBS_URL . "__{$width}__{$height}__{$crop}__{$last_modified}.{$format}";
	
	
	
	// if exist before then return the image
	if ( file_exists(BASE_URL.$thumb_file) ) return $thumb_file;
	...
	// save file
	if ($format == 'png') {
		imagepng($dst_img, BASE_URL.$thumb_file, 9);
	}	
	elseif ($format == 'jpg') {
		imagejpeg($dst_img, BASE_URL.$thumb_file, 85);
	}
